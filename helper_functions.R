...
build_phylo_obj <- function(selected_patients, counts_all) {
  ...
  counts_all <- counts_all %>% 
    dplyr::select("Kingdom","Phylum","Class","Order","Family","Genus","Species", any_of(selected_patients$SampleID))
  ...
  counts_all<-counts_all%>%mutate(Kingdom=paste0("d__",Kingdom),
...
run_mas_all_levels <- function(phylo_obj,variable){
  ...
  for (tax_level in tax_levels){
    ...
    fit_mas <- Maaslin2(input_data = df_input_data, 
...
combine_res_mas_lind <- function(mas_res,linda_res){
  ...
  m <- mas_res %>% dplyr::select(feature, name, coef, stderr, pval) %>% 
    dplyr::rename(log2FoldChange_mas=coef, lfcSE_mas=stderr, pvalue_mas=pval) %>%
    mutate(feature = str_replace_all(feature,"\\."," ")) %>% 
    mutate(feature = str_replace_all(feature,"\\-"," ")) 
  
  # Here we have to correct the taxa names becuase MaAsLin does weird stuff with them
  linda_res2 <- linda_res %>% dplyr::select(-lin_bhcor_p) %>% rownames_to_column(var="feature")
  linda_res_long <- linda_res2 %>% pivot_longer(-feature, names_sep = "\\.", names_to = c("name",".value"))
  linda_res_long2 <- linda_res_long %>% dplyr::select(feature, name, log2FoldChange, lfcSE, pvalue) %>%
    dplyr::rename(log2FoldChange_lin=log2FoldChange, lfcSE_lin=lfcSE, pvalue_lin = pvalue) %>%
    mutate(feature = str_replace_all(feature,"\\."," ")) %>% 
    mutate(feature = str_replace_all(feature,"\\-"," ")) %>% 
    mutate(feature = str_replace_all(feature,"\\["," ")) %>%
    mutate(feature = str_replace_all(feature,"\\]"," ")) 
  ...
  combined <- dplyr::full_join(m, linda_res_long2, by=c("feature","name"))
  # here we get rid of antibiotics, dont need to include them in pvalue calc
  # THIS LINE NEEDS TO BE MANUALLY SET
  combined2 <- combined %>% filter(!str_detect(name,"new_anti")) %>% filter(!str_detect(name,"facility"))%>% filter(!str_detect(name,"t_simple"))
  
  # pivot wider and apply couchy combination & FDR
  combined_wp <- combined2 %>% mutate(couchy_t = 0.5*((tan((0.5-pvalue_lin)*pi)) + tan((0.5-pvalue_mas)*pi)) )
  combined_wp <- combined_wp %>% mutate(couchy_p = 0.5-(atan(couchy_t)/pi) )
